...
- Add semicolon or null byte before the file extension like `image.php%00.png`
- Use multibyte unicode characters that convert to dots or nullbytes after normalization.
#### Validation of file contents
Some servers validate the contents of the file. for example, jpg files start with `FF D8 FF` and if the file provided doesn't have these bytes the server might reject it. if the check is this simple you can easily bypass it by inserting a comment containing a malicious code to the image, or you can upload a php file but use the hex editor in burp to insert the [Magic Number](https://gist.github.com/leommoore/f9e57ba2aa4bf197ebc5).
#### Race condition
...
If the uploading and serving of the file is secure. Your last bet is to check the parsing of the files requested, if you can upload files like .doc, .xls,.svg or .xml you can try XXE injection attacks.
#### Uploading files using PUT method
	If the server supports the PUT method, it's worth to check if you can uplozad a malicious file and execute it:
```
...
### Resources
Check the academy for practice at [Portswigger](https://portswigger.net/web-security/file-upload)
